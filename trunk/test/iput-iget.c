...
#define PE1_PUTS 0	// 0 is the case which fails, results[] not written?
			// if 1, the PE1 puts --> Results[0...4] which works
			// validates data correct in PE1
...
main(int argc, char **argv)
{
	...
	if (me == 0) {
		...
		shmem_short_get(results, target, 5, 1);
#endif
		shmem_barrier_all();
...
#endif
#if PE1_PUTS == 1
		shmem_short_put(results, target, 5, 0);
#endif
		shmem_barrier_all();
	}
