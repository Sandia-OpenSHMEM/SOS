...
shmem_transport_addr_t shmem_transport_get_local_addr(void)
{
    ...
    int ret = PtlGetPhysId(shmem_transport_portals4_ni_h, &ptl_addr);
    if (PTL_OK != ret) {
        RAISE_ERROR_MSG("PtlGetPhysId failed: %d\n", ret);
    }

    addr.addrlen = sizeof(ptl_addr);
    addr.addr = &ptl_addr;
...
int shmem_transport_same_node(const shmem_transport_addr_t *a1, const shmem_transport_addr_t *a2) {
    ptl_process_t *ptl_addr1, *ptl_addr2;

    ptl_addr1 = (ptl_process_t *) a1->addr;
    ptl_addr2 = (ptl_process_t *) a2->addr;

    if (ptl_addr1->phys.nid == ptl_addr2->phys.nid) {
        return 1;
    } else {
        return 0;
    }
